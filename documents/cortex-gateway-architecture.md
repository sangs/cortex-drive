...
While Nginx is faster for pure routing, `cortex-gateway` allows for **application-aware orchestration**. It can parse the body of incoming requests, interact with auth providers programmatically, and (as planned) integrate LLMs to transform natural language into specific tool calls before proxying.
