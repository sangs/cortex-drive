...
with driver.session() as session:
    # 1. Detach Delete phantom nodes (no labels, no properties)
    print("Deleting phantom nodes with DETACH DELETE...")
    res_del = session.run("MATCH (n) WHERE labels(n) = [] AND properties(n) = {} DETACH DELETE n")
