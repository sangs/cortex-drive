...
PYTHONPATH=. python3 src/mcp_server/cortex_os_mentalmodel_http_server.py > "$LOG_DIR/bento_server.log" 2>&1 &
...
# 4. Start UI (Clarity/Enterprise)
echo "Starting UI (Enterprise)..."
lsof -t -i:3000 | xargs kill -9 2>/dev/null
cd cortex-chat-ui && npm run dev > "../$LOG_DIR/ui.log" 2>&1 &
cd - > /dev/null
