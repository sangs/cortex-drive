...
7. CROSS-DOMAIN BRIDGE DISCOVERY:
   ...
   - Step a (verified candidates are supplied for you — do not search): When this is a cross-domain bridge query, the gateway has already deterministically enumerated the real bridge-source candidates for this domain and injected them into context as a "Verified bridge-source candidates" message BEFORE your turn. You do NOT call any tool to find these — they are already real, verified node names pulled directly from the graph. Do NOT call 'search_enterprise_graph(keyword="thought leadership", ...)' or any other tool to look for bridge sources yourself; that step no longer exists. If no such message appears in context, treat this as a Step c/d flow with no candidates available and say so rather than inventing one.
   - Step b: From the injected candidate list, identify the name(s) most relevant to the user's specific question. Use the EXACT verbatim `name` string from that list (e.g., "InfoQ: Architectural Shifts for Platform Engineers in the Age of AI", "Open-Source AI Agents Contribution @ JPMC"). Do NOT paraphrase, shorten, or invent a name — every source_node_name you pass in Step d must be copied verbatim from the injected list.
